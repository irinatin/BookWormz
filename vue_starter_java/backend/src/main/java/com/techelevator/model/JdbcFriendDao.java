...
public class JdbcFriendDao implements FriendDAO{
	...
	public List<Friend> getAllFriends(long userId) {
		...
		String sqlGetFriends = "SELECT users.username, user_info.first_name, user_info.last_name FROM users "
				+ "JOIN user_info ON users.id = user_info.user_id JOIN friends ON users.id = friends.friend_id "
