...
export default {
  ...
  methods: {
    getBooks() {
       axios.get(`${process.env.VUE_APP_REMOTE_API}/api/getAllBooks`, {
    ...
    selectUser() {
      axios.get(`${process.env.VUE_APP_REMOTE_API}/api/getUser`, {
      headers:{"Authorization" :  'Bearer ' + localStorage.getItem('Authorization')}})
    .then(response => {
      console.log(response)
    ...
    addReadingEvent() {
       axios.post(`${process.env.VUE_APP_REMOTE_API}/api/addReadingEvent`, this.readingEvent, {
      headers:{"Authorization" :  'Bearer ' + localStorage.getItem('Authorization')}})
    .then(response => {
      console.log(response)
    })
